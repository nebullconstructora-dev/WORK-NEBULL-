document.addEventListener("DOMContentLoaded", () => {
  ...
  // Compartir perfil (Web Share API / copia link)
  const shareBtn = document.getElementById("shareBtn");
  shareBtn?.addEventListener("click", async () => {
    const url = window.location.href.split("#")[0];
    const title = document.title || "Constructora NEBULL";
    const text = "Perfil oficial: Constructora NEBULL";
    ...
    try {
      ...
      alert("Link copiado. ¡Ya puedes pegarlo y compartir!");
    } catch (_) {
      // Fallback final
